...
class ContactService {
  ...
  async submitContact(contactData) {
    ...
    try {
      const data = await verifyCaptcha(contactData.captchaToken);

      if (!data.success || data.score < 0.5) {
        throw customError("Failed reCAPTCHA verification", 400);
      }
  ...
  async getAllContacts(filters, page = 1, limit = 10) {
    return await contactRepository.paginate(filters, page, limit, {
      createdAt: -1,
    });
  }
