...
class OrderRepository extends BaseRepository {
    ...
    checkProductExistWithQuantity = async (products, session) => {
        ...
        const dbProducts = await Product.find({ _id: { $in: ids } })
            .populate([
                { path: "inventory", select: "_id quantity status" }
            ])
            .session(session);
        ...
        for (const item of products) {
            const product = dbProducts.find((p) => String(p._id) === String(item.product));

            if (!product.status) {
                throw customError(`${product.name} is not available, please remove it`, 400);
            }
     ...
     */
    updateProductInventory = async (products, session, direction = "decrease") => {
