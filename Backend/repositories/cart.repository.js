...
class CartRepository extends BaseRepository {
    ...
    async addOrUpdateItem(userId, productId, quantity = 1, session = null) {
        ...
        if (inventory.status === STOCK_STATUS.PRE_ORDER && quantity <= 0) {
            throw customError(`Pre-order requires minimum 1 unit for ${product.name}.`);
        }
    ...
    async syncCart(userId, localItems, session) {
        ...
        for (const { productId, quantity } of localItems) {
            ...
            const { quantity: stockAvailable, status: stockStatus } = product.inventory;
            ...
            if (existingItem) {
                ...
                } else if (stockStatus === STOCK_STATUS.IN_STOCK && quantity > stockAvailable) {
                    throw customError(`Only ${stockAvailable} units available for ${product.name}.`);
                } else {
                    existingItem.quantity = quantity;
                }
            } else {
                if (quantity > 0) {
                    if (stockStatus === STOCK_STATUS.IN_STOCK && quantity > stockAvailable) {
                        throw customError(`Only ${stockAvailable} units available for ${product.name}.`);
                    }
                    cart.items.push({ product: productId, quantity });
                }
