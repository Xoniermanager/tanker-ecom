  ...
  return (
    <>
      <div
        ...
        className="py-22 md:py-28 px-6 w-full bg-no-repeat bg-cover"
      >
        <div className="max-w-7xl mx-auto flex flex-col gap-14">
          <div className="flex flex-col gap-4 items-center">
            ...
            <h2 className="font-black text-5xl lg:text-7xl text-purple-950 md:w-1/2 text-center capitalize">
              {workProcessData?.heading || "N/A"}
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-30 lg:gap-10">
...
<div className="flex flex-col justify-between items-center relative group" key={i}>
              <div style={{backgroundImage: "url('/images/services-bg.jpg')"}} className="p-8 pb-26 bg-cover border-1 process-card relative border-[#e7e7e7] rounded-[40px] rounded-tl-[0px] group-hover:rounded-tl-[40px] group-hover:rounded-tr-[0px] w-full z-20 flex items-center flex-col justify-center gap-5">
                <h3 className="text-2xl font-black text-purple-950 capitalize text-center">{item?.title || "N/A"}</h3>
                <p className="text-center text-[#6e797b] font-medium ">{item?.description || "N/A"}</p>
                <span className="bg-black hidden md:block text-white p-0.5 px-1 rounded-md font-bold text-sm uppercase tracking-wide absolute left-5 -bottom-2">{item?.subtitle || "N/A"}</span>
              </div>
